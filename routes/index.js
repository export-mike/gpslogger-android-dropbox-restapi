...
router.get('/auth/dropbox', (req, res) => {
  ...
  if (!req.query.code) {
    dropbox.Authenticate(process.env.DROPBOX_KEY, process.env.DROPBOX_SECRET, `${req.protocol}://${req.get('host')}/auth/dropbox`, (err, url) => {
      if (err || !url) {
    ...
    dropbox.AccessToken(
      ...
      req.query.code,
      `${req.protocol}://${req.get('host')}/auth/dropbox`,
      (err, account) => {
        if (err) {
